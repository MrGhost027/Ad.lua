...
game:GetService("Workspace").PlayerDraggables[game.Players.LocalPlayer.UserId].DescendantAdded:Connect(function(added)
	...
	if added:IsA("Part") and added:FindFirstChild("Coins") ~= nil then
		...
		repeat
			wait(0.1)
			firetouchinterest(added, game:GetService("Workspace").Areas["Area18_Apartment"].Goal, 0)
			firetouchinterest(added, game:GetService("Workspace").Areas["Area18_Apartment"].Goal, 1)
		until added == nil
	end
end)
...
for _, v in pairs(area.DraggableItems:GetDescendants()) do
	if v:IsA("StringValue") and v.Name == "Title" and v.Value == "Tub" then
...
autofarm:addToggle("Enabled", nil, function(value)
	_G.On = value
	HumanoidRootPart.CFrame = CFrame.new(-122, 12, 12208)
	wait(0.1)
	task_defer(function()
